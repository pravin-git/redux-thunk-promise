...
it("userReducer", () => {
  ...
  expect(userReducer(undefined, actionPending)).toEqual({
    error: null,
    fetched: false,
    fetching: true,
    currentUsers: [],
  ...
  expect(userReducer(undefined, actionUserReceived)).toEqual({
    error: null,
    fetched: true,
    fetching: false,
    currentUsers: [],
    users: actionUserReceived.payload
  });

  const actionUserFulfilled = { type: "User_FULFILLED", payload: [] };
  expect(userReducer(undefined, actionUserFulfilled)).toEqual({
    error: null,
    fetched: true,
    fetching: false,
    currentUsers: [],
    users: actionUserFulfilled.payload.data
  });

  const actionError = { type: "Fetch_users_error", payload: [] };
  expect(userReducer(undefined, actionError)).toEqual({
...
it("user component when users are there", () => {
  const wrapper = shallow(
    <UserComponent users={[{ name: "pravin" }]} showHeader={true} />
  );
